...
The core functions are `dtruncpois()` (density), `ptruncpois()` (CDF), `qtruncpois()` (quantiles), and `rtruncpois()` (random generation). Alongside these, `extruncpois()`, `vartruncpois()`, `medtruncpois()`, and `modtruncpois()` give the mean, variance, median, and mode.
